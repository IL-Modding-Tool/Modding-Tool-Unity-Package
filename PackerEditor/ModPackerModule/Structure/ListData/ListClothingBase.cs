...
namespace ModPackerModule.Structure.ListData
{
    ...
    {
        public ListClothingBase(in SideloaderMod.SideloaderMod sideloaderMod, in XElement element, string category) :
            base(in sideloaderMod, in element, category)
        {
            ...
            TextureBundle = element.Attr("tex-bundle",
                GetBundleFromName(Texture) ?? GetBundleFromName(TextureMask) ?? "0");

            sideloaderMod.CharacterInfo.RememberCharacterItem(this);
        }


        public override string[] ThumbnailGenerationTargetAssets()
        {
            return new string[] {MeshAsset};
        }
