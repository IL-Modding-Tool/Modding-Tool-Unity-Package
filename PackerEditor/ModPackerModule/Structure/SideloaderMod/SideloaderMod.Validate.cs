    ...
    public partial class SideloaderMod
    {
        ...
        private void ValidateBundleData()
        {
            ...
            foreach (var bundleTarget in _bundleTargets)
            {
                ...
                confirmed = false;
                Issues.Add(new Issue(Error, AssetBundleReference, $"{bundleTarget}", reason));
            }

            if (confirmed) return;
            throw new InvalidBundleTargetException($"Found bad bundle targets from the {FileName} file.");
        }

        private void ValidateListData()
        {
            var confirmed = true;
            foreach (var list in GameItems.Values.SelectMany(info => info.ItemList.Values.SelectMany(x => x)))
            {
                var result = list.IsValid(out var reason);
                if (result) continue;
                Issues.Add(new Issue(Error, Issue.IssueType.BuildTarget, $"Asset \"{reason}\" is not found in bundle targets", list.ToString()));
                confirmed = false;
            }

            if (confirmed) return;
            throw new InvalidListTargetException($"Found bad list targets from the {FileName} file.");
        }
