    ...
    public partial class SideloaderMod
    {
        ...
        private IEnumerable<AssetBundleBuild> SplitBundleIntoModules(IEnumerable<AssetBundleBuild> builds)
        {
                        ...
                        return bundle.assetNames
                            .SelectMany(asset =>
                                AssetDatabase.GetDependencies(asset)
                                    .Where(assetName =>
                                        !parentAssets.ContainsKey(assetName)) // it should stay where it belongs.
                                    .Select(dependantAsset => (Asset: dependantAsset,
                                        Group: Path.GetDirectoryName(dependantAsset)))
                            );
                    }
                )
                ...
                .GroupBy(tuple => tuple.Group))
            {
                ...
                bundlePath =
                    Path.Combine(DependencyData.Path, bundlePath).ToUnixPath(); // Dependency Data's value is ensured.
        ...
        private void SetupFolder(string targetPath, bool isDryRun = false)
        {
            foreach (var targetName in Assets.GetTargetBundles)
            {
                ...
                using (var writer = XmlWriter.Create(sb, settings))
                {
                    if (isMainArchive)
                    {
                        OutputDocumentObject.Save(writer);
                    }
                    else
                    {
                        var info = XmlUtils.GetManifestTemplate();
                        MainData.SaveData(ref info);
                        info.Root?.Element("guid")
                            ?.SetValue(MainData.guid + "." + targetName.SanitizeNonCharacters("."));
                        info.Root?.Element("name")
                            ?.SetValue(MainData.name + " " + targetName.SanitizeNonCharacters("."));
                        info.Save(writer);
                    }
                }
        ...
        private void Deploy(string targetPath)
        {
            try
            {
                ...
                void Progress()
                {
                    EditorUtility.DisplayProgressBar("Compressing...", "Compressing and deploying zipmod",
                        progressDone / (float) progressMAX);
                }
        ...
        public bool CreateStudioThumbnails()
        {
            var targetObjects = StudioGameObjects;
            if (targetObjects.IsNullOrEmpty()) return false;
            HoohTools.Instance.GenerateStudioThumbnail(this);
            return true;
        }

        public bool CreateItemThumbnails()
        {
            var targetObjects = CharacterGameObjects;
            if (targetObjects.IsNullOrEmpty()) return false;
            HoohTools.Instance.GenerateThumbnail(targetObjects, PathThumbnails);
            return true;
        }
    }
}